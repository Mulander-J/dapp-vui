...
import { Metamask, MetaMaskProvider, MetaMaskProviderRpcError } from '@/plugins/wallets/metamask'
import { Walletconnect, WalletConnectProvider } from '@/plugins/wallets/walletconnect'
import {
  Walletlink,
  WalletLinkProvider,
  WalletLinkProviderRpcError,
} from '@/plugins/wallets/walletlink'
import { useEthers } from './useEthers'
import { INFURA_API } from '@/constants'

export type WalletProvider = MetaMaskProvider | WalletConnectProvider | WalletLinkProvider
 ...
 */
// const onDisconnectCallback = ref<OnDisconnectCallback | null>((msg: string) => {
//   console.log('disconetEEEE', msg)
// })
// const onAccountsChangedCallback = ref<OnAccountsChangedCallback | null>((accounts: string[]) => {
//   console.log('accounts', accounts)
// })
// const onChainChangedCallback = ref<OnChainChangedCallback | null>((chainId: number) => {
//   console.log('chainId', chainId)
// })

export function useWallet(options: UseWalletOptions = { library: 'ethers' }) {
  const { activate, deactivate, address } = useEthers()
  ...
  async function connect(_walletName: WalletName, infuraAPI?: string, appName?: string) {
    ...
    try {
      ...
      switch (_walletName) {
        ...
        case 'walletconnect':
          ...
          _provider = (await Walletconnect.connect(infuraAPI)) as WalletConnectProvider
          if (!_provider.connected) throw new Error('walletconnect is not connected')
          break
        case 'walletlink':
          if (!infuraAPI) throw new Error('You should provide infuraAPI for connecting WalletLink')
          if (!appName) throw new Error('You should provide an app name for connecting WalletLink')
          _provider = (await Walletlink.connect(infuraAPI, appName)) as WalletLinkProvider
    ...
    try {
      options.library === 'ethers' && (await activate(provider.value as WalletProvider))
    } catch (err: any) {
      clear()
  ...
  function subscribeChainChanged() {
    switch (walletName.value) {
      ...
      case 'walletconnect':
        ...
        break
      case 'walletlink':
        ;(provider.value as WalletLinkProvider).on('chainChanged', async (hexChainId: string) => {
          status.value = 'connecting'
          // Changing network might lead to disconnect so the provider would be deleted.
          if (!provider.value) {
            error.value = `Failed when changing chain: missing provider`
            status.value = 'none'
            await connect('walletlink', INFURA_API)
            return
          }
  ...
  const isLoading = computed(() => {
    if (status.value === 'connecting' || (status.value === 'connected' && !address.value))
      return true
    else return false
  })
